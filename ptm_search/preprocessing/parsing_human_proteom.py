...
def get_PTMs_lists(query_text, file_2, list_of_msms_proteins):
    PTMs_groups = {}
        ...
        for i in range(0, len(description_list)):
            ...
            if 'AC   ' in description_list[i]:
                for elm in description_list[i].split()[1:]:
                    if elm[:-1] in list_of_msms_proteins.keys():
                        ...
                        file_2.write('// ' + f'{acc_of_protein}|{list_of_msms_proteins[acc_of_protein]}' + '\n')
                        file_2.write('\n')
                        break
            if acc_of_protein == '':
            ...
            if 'FT   MOD_RES' in description_list[i]:
                ...
                if acc_of_protein not in PTMs_groups[correct_prot_modif_name].keys():
                    ...
                    file_2.write(correct_prot_modif_name + '\n')
                    file_2.write('\n')
                    continue
...
def parsing_human_proteom(config, dataframe):
    ...
    # file2 = open(os.path.join(config.ptm_search_dir, f'{config.experiment_name}_MOD_RES_{config.analysis_index}.txt'), 'w')
    print(os.path.join(config.ptm_search_dir, f'{config.experiment_name}_MOD_RES_{config.analysis_index}.txt'))

    # prot_acc_and_names = get_all_accs_and_names(dataframe)
    # return get_PTMs_lists(query_text, file2, prot_acc_and_names), prot_acc_and_names